...
import PageContextEngine from "./context/page-context-engine.js";
import ModelRouter from "./agent/model-router.js";
import ObservationBuilder from "./agent/observation-builder.js";
...
//----------------------------------------------------------
// Phase 2: Page Context Engine & Model Router
//----------------------------------------------------------

const pageContextEngine = new PageContextEngine({
  debug: CONFIG.debug,
  enableSoM: true,
});
...
// Register Phase 2 tools on toolMap
toolMap.register(
  "observe",
  async (args = {}) => {
    const page = await ensureBrowserReady();
    return await pageContextEngine.createObservation(page, args);
  },
  {
    category: "observation",
    description: "Capture unified page observation (DOM, A11y, Screenshot, Set-of-Marks).",
  },
);
...
toolMap.register(
  ...
  async (args = {}) => {
    const page = await ensureBrowserReady();
    if (args?.som) {
      const elements = await pageContextEngine.extractInteractiveElements(page);
      return await pageContextEngine.generateSetOfMarks(page, elements);
    }
    return await pageContextEngine.captureScreenshot(page);
  },
  {
    category: "observation",
    description: "Capture viewport screenshot (clean or Set-of-Marks annotated).",
  },
);

// Configure Jarvis Generic Agent Engine with model & browser services
...
//==========================================================
// PHASE 2: PAGE CONTEXT & AGENT OBSERVATION ENDPOINTS
//==========================================================

app.get("/context/observation", async (req, res) => {
  try {
    const page = await ensureBrowserReady();
    const includeScreenshot = req.query.screenshot !== "false";
    const enableSoM = req.query.som === "true";
    const observation = await pageContextEngine.createObservation(page, {
      includeScreenshot,
      enableSoM,
    });
    ...
    return res.json(
      success({
        observation,
        summary: ObservationBuilder.summarize(observation),
      }),
    );
  } catch (err) {
    return res.status(500).json(failure(err));
...
app.get("/context/screenshot", async (req, res) => {
  try {
    const page = await ensureBrowserReady();
    const som = req.query.som === "true";
    let result;
    if (som) {
      const elements = await pageContextEngine.extractInteractiveElements(page);
      result = await pageContextEngine.generateSetOfMarks(page, elements);
      return res.json(
        success({
          screenshot: result.somDataUri,
          markedCount: result.markedElements.length,
        }),
      );
    } else {
      result = await pageContextEngine.captureScreenshot(page);
      return res.json(
        success({
          screenshot: result.dataUri,
...
app.post("/agent/observe", async (req, res) => {
  try {
    ...
    const observation = await pageContextEngine.createObservation(page, options);
...
app.post("/agent/plan", async (req, res) => {
  try {
    const page = await ensureBrowserReady();
    const goal = String(req.body?.goal || req.body?.command || "").trim();
    if (!goal) {
      return res.status(400).json(failure(new Error("Missing goal or command.")));
    }

    const observation = await pageContextEngine.createObservation(page, {
      includeScreenshot: true,
      enableSoM: req.body?.useVisualSoM ?? true,
    });

    const planDecision = await modelRouter.planAction(observation, goal, {
      useVisualSoM: req.body?.useVisualSoM ?? true,
    });
...
//==========================================================
// JARVIS AUTONOMOUS AGENT ENGINE ROUTES
//==========================================================

// 1. RUN / START GENERIC AGENT GOAL (Flights, Calendar, Notes, Maps, Shopping, Research)
app.post("/agent/run", async (req, res) => {
  try {
    const goal = String(req.body?.goal || req.body?.command || "").trim();
    if (!goal) {
      return res.status(400).json(failure(new Error("Missing goal or command parameter.")));
    }

    // Launch agent execution asynchronously in the engine
    ...
    return res.json(
      success({
        message: "Agent goal accepted and execution started",
        goal,
        state: agentEngine.getState(),
      }),
    );
  } catch (err) {
    return res.status(500).json(failure(err));
...
// 2. LIVE SSE EVENT STREAM FOR AGENT (STEPS, REASONING, QUESTIONS)
app.get("/agent/events", (req, res) => {
  agentEngine.subscribe(res);
});

// 3. GET CURRENT AGENT STATE SNAPSHOT
app.get("/agent/state", (req, res) => {
  return res.json(success(agentEngine.getState()));
});

// 4. ANSWER HUMAN-IN-THE-LOOP CLARIFICATION QUESTION
app.post("/agent/answer", async (req, res) => {
  try {
    const { questionId, answer } = req.body || {};
    if (!answer) {
      return res.status(400).json(failure(new Error("Missing answer parameter.")));
    }
    const state = await agentEngine.answerQuestion(questionId, answer);
    return res.json(
      success({
        message: "Answer submitted successfully, execution resumed",
        state,
      }),
    );
  } catch (err) {
...
app.post("/agent/pause", (req, res) => {
  const state = agentEngine.pause();
  return res.json(success({ message: "Agent execution paused", state }));
});

// 6. RESUME AGENT
app.post("/agent/resume", (req, res) => {
  const state = agentEngine.resume();
  return res.json(success({ message: "Agent execution resumed", state }));
});

// 7. STOP AGENT
app.post("/agent/stop", (req, res) => {
  const state = agentEngine.stop();
  return res.json(success({ message: "Agent execution stopped", state }));
});

// 8. MANUAL ADVANCE STEP
app.post("/agent/step", async (req, res) => {
  try {
    const state = await agentEngine.manualStep();
    return res.json(success({ message: "Advanced step", state }));
  } catch (err) {
    return res.status(500).json(failure(err));
  }
});

// 9. RESOLVE PLANNER CHECK (HUMAN-IN-THE-LOOP CHECKPOINT)
app.post("/agent/planner-check", async (req, res) => {
  try {
    const { checkId, action, instruction } = req.body || {};
    const state = await agentEngine.resolvePlannerCheck(checkId, action || "resolved", instruction || "");
    return res.json(
      success({
        message: "Planner check resolved and execution resumed",
        state,
      }),
    );
  } catch (err) {
...
// 10. PROVIDE MISSING CREDENTIALS PROMPTED BY AGENT
app.post("/agent/credential-response", async (req, res) => {
  try {
    const { requestId, username, password, storage, extraFields } = req.body || {};
    if (!username || !password) {
      return res.status(400).json(failure(new Error("Username and password are required.")));
    }
    const state = await agentEngine.provideCredentials(requestId, username, password, {
      storage: storage || "both",
      extraFields: extraFields || {},
    });
    return res.json(
      success({
        message: "Credentials saved and login resumed",
        state,
      }),
    );
  } catch (err) {
...
//==========================================================
// CREDENTIAL VAULT API (LOCAL ENCRYPTED & FIREBASE FIRESTORE)
//==========================================================

// GET ALL SAVED CREDENTIALS (PASSWORDS MASKED)
app.get("/api/credentials", async (req, res) => {
  try {
    const list = await credentialManager.list();
    return res.json(success({ credentials: list, count: list.length }));
  } catch (err) {
    return res.status(500).json(failure(err));
  }
});

// SAVE / UPDATE CREDENTIAL (LOCAL, FIREBASE, OR BOTH)
app.post("/api/credentials", async (req, res) => {
  try {
    const { site, username, password, storage, extraFields } = req.body || {};
    if (!site || !username || !password) {
      return res.status(400).json(failure(new Error("site, username, and password are required.")));
    }
    const result = await credentialManager.save(site, username, password, {
      storage: storage || "both",
      extraFields: extraFields || {},
    });
    return res.json(success({ message: "Credential stored securely", result }));
  } catch (err) {
    return res.status(500).json(failure(err));
  }
});
...
// DELETE CREDENTIAL
app.delete("/api/credentials/:site", async (req, res) => {
  try {
    const { site } = req.params;
    const storage = req.query.storage || "both";
    const result = await credentialManager.remove(site, { storage });
    return res.json(success({ message: `Credential removed for "${site}"`, result }));
  } catch (err) {
    return res.status(500).json(failure(err));
  }
});

// SYNC CREDENTIALS (LOCAL <-> FIREBASE FIRESTORE)
app.post("/api/credentials/sync", async (req, res) => {
  try {
    const result = await credentialManager.sync();
    return res.json(success({ message: "Sync operation completed", result }));
  } catch (err) {
    return res.status(500).json(failure(err));
  }
});
